...
- `iniciar <usuario> <clave>`: inicia sesión; si no existe, registra.
- `salir`: cierra sesión.
- `perfil`: muestra monedas acumuladas actuales, inventario resumido.
...
- `crear_sala tiempo_turno=20`: crea sala.
...
- `movimientos`: lista de máximo 4 movimientos; cada uno tiene nombre, tipo y poder.
...
- Precio base recomendado de sobres:
  - básico: 100
  - avanzado: 250
- Si un pokémon sale repetido, debe elegir en su implementación:
  - opción A: sobreescribir el que ya existe,
...
### 5.6 Validación de movimientos en combate

- El comando `ataque <movimiento>` solo es válido si el movimiento pertenece al conjunto de movimientos del Pokémon activo.
- Si el movimiento no existe o no lo tiene el Pokémon activo, el sistema debe responder con un mensaje de error y solicitar al jugador que elija de nuevo (no consume el turno).

### 5.7 Tipos (tabla mínima requerida)
...
### 5.8 Estadísticas, nivel y evolución
...
7. Ana crea sala:
   - comando: `crear_sala tiempo_turno=20`
