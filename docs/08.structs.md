...
Un **protocolo** es un mecanismo de Elixir para lograr **polimorfismo**: permite definir un conjunto de funciones que pueden tener **implementaciones distintas según el tipo de dato** que reciben. Es la respuesta funcional de Elixir a la pregunta "¿cómo hago que la misma función se comporte distinto para diferentes tipos?".

Un ejemplo que ya usamos sin saberlo es el protocolo `String.Chars`, que es el que permite que `IO.puts/1` y la interpolación `#{}` conviertan valores a texto. Por eso `IO.puts(42)` funciona: existe una implementación de `String.Chars` para los enteros.

### Definir un protocolo

Supongamos que queremos una función `describir/1` que funcione distinto para distintos structs:

```elixir
defprotocol Describible do
  @doc "Devuelve una descripción en texto del dato"
  def describir(dato)
end
```
...
Ahora damos una implementación para cada tipo con `defimpl`:
...
defmodule Producto do
  defstruct [:nombre, :precio]
end

defimpl Describible, for: Usuario do
  def describir(%Usuario{nombre: nombre, edad: edad}) do
    "Usuario #{nombre}, #{edad} años"
  end
end

defimpl Describible, for: Producto do
  def describir(%Producto{nombre: nombre, precio: precio}) do
...
```elixir
Describible.describir(%Usuario{nombre: "Ana", edad: 30})
# => "Usuario Ana, 30 años"

Describible.describir(%Producto{nombre: "Laptop", precio: 1200})
# => "Producto Laptop: $1200"
```

La misma llamada `Describible.describir/1` ejecuta código diferente según el tipo recibido. Esto se conoce como **polimorfismo basado en protocolos**.

> ⚠️ **Protocolos vs. structs:** Los structs definen la *forma* de los datos; los protocolos definen *comportamientos* que pueden implementarse para muchos tipos distintos. Protocolos importantes de la librería estándar son `Enumerable` (lo que permite usar `Enum`), `Collectable` (usado por la opción `:into`) e `Inspect` (usado por `IO.inspect`).

----

## Para la próxima clase

- Repase e implemente por su cuenta el ejemplo de protocolos visto en esta guía.
