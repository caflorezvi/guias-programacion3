...
- Tipos de datos, átomos y tuplas.
- Estructuras de decisión (`if`, `cond` y `case`) y *guards*.
- Coincidencia de patrones (pattern matching).
- Ejemplos prácticos.

En la siguiente guía se retomarán estos conceptos para trabajar el manejo de resultados con tuplas y la estructura `with`.
...
Los operadores lógicos, matemáticos y de comparación se agrupan así:
...
Para más información sobre átomos y tuplas, puede consultar [documentación de átomos](https://hexdocs.pm/elixir/Atom.html) y [documentación de tuplas](https://hexdocs.pm/elixir/Tuple.html).
...
defmodule Descuento do
  ...
  defp leer_precio do
    IO.gets("Ingrese el valor de la compra: ")
    |> String.trim()
    |> String.to_float()
  end
...
Guarde el archivo como `descuento.exs` y ejecútelo con `elixir descuento.exs`. La sesión se ve así, donde lo que aparece después de los dos puntos es lo que usted escribe:

```
Ingrese el valor de la compra: 120000.0
El precio original es: 1.2e5, el descuento aplicado es: 10.0%, y el precio final es: 1.08e5
```

Pruebe con valores de cada rango para comprobar que los cuatro descuentos funcionan:

```
Ingrese el valor de la compra: 30000.0
El precio original es: 3.0e4, el descuento aplicado es: 0%, y el precio final es: 3.0e4
```

```
Ingrese el valor de la compra: 60000.0
El precio original es: 6.0e4, el descuento aplicado es: 5.0%, y el precio final es: 5.7e4
```

Para leer el precio se reutiliza lo visto en la guía anterior (`IO.gets/1` y `String.trim/1`) y se agrega `String.to_float/1`, que convierte una cadena en un número decimal.

> ⚠️ **Importante:** `String.to_float/1` exige que la cadena incluya los decimales. Si el usuario escribe `75000`, el programa **falla** con un `ArgumentError`; debe escribir `75000.0`. Más adelante veremos cómo resolver esto de forma segura.

Al ejecutarlo notará también que los valores grandes se imprimen en **notación científica**: al ingresar `200000.0` el mensaje muestra `2.0e5` en lugar de `200000.0`. Así representa Elixir los números decimales grandes. Por ahora no afecta el resultado del cálculo; en un ejemplo posterior se verá cómo darles formato.
...
Ejecute el programa con los mismos valores de la Versión 1 y compruebe que **la salida es idéntica**. Cambió la forma de escribir la decisión, no el resultado.
...
Para más información puede leer acerca de [cond](https://hexdocs.pm/elixir/case-cond-and-if.html#cond) y [guard](https://hexdocs.pm/elixir/patterns-and-guards.html#guards) en la documentación oficial de Elixir, allí puede profundizar en el uso de estas estructuras de control.

Con estas tres versiones se resolvió el problema de decidir entre varios rangos. Sin embargo, queda pendiente el punto débil que se advirtió al inicio: si el usuario no escribe el número exactamente como `String.to_float/1` lo espera, el programa se cae. Ese será el problema del siguiente ejemplo.

---

## Ejemplo 2

En el ejemplo anterior quedó un problema sin resolver: una entrada mal escrita hace fallar el programa. Este ejemplo lo aborda de frente.
...
Ingrese el valor por hora: 20000.0
El salario de Ana es: $9.0e5
```

El valor por hora se escribe con decimales (`20000.0`). Si escribe `20000` el programa se cae, que es justamente el problema que este ejemplo va a resolver:

```
Ingrese el valor por hora: 20000
** (ArgumentError) errors were found at the given arguments:

  * 1st argument: not a textual representation of a float

    :erlang.binary_to_float("20000")
```
...
Ejecute ahora el programa y escriba algo que no sea un número. Compare el resultado con el `ArgumentError` de la Versión 1:

```
Ingrese el nombre del empleado: Carlos
Ingrese las horas trabajadas: 45
Ingrese el valor por hora: abc
Error. Se utilizará 0.0 como valor predeterminado.
El salario de Carlos es: $0.0
```

El programa ya no se cae. Avisa del problema, usa el valor por defecto y sigue adelante, que era el objetivo de este ejemplo.
...
defmodule Util do
  ...
  def leer(mensaje, :float) do
    ...
    case valor do
      {numero, _} -> numero
      :error ->
        imprimir_error("Error. Se utilizará 0.0 como valor predeterminado.")
        0.0 # Se devuelve 0.0 para conservar el tipo flotante
    end
  end
...
La segunda forma usa una **función anónima**, como las que se estudiaron en la guía de programación funcional. Ambas hacen lo mismo, así que use la que le resulte más legible.
...
En el módulo `Salario` de la Versión 4, **reemplace `generar_mensaje/2`** por estas dos funciones. El resto del módulo no cambia.
...
Con el mismo dato que en la Versión 1, el salario ya se lee sin notación científica:

```
Ingrese el nombre del empleado: Carlos
Ingrese las horas trabajadas: 45
Ingrese el valor por hora: 20000.0
El salario de Carlos es: $900000.00
```

---

## Reutilización del módulo `Util`

> ⚠️ **Importante:** todos los ejemplos que siguen utilizan el módulo `Util` construido en el Ejemplo 2. Realice estos pasos **antes de continuar**; de lo contrario, ninguno de los programas siguientes funcionará.

1. Cree un archivo llamado `util.exs` y copie en él el código del módulo `Util` de la Versión 5.
...
4. Se generará un archivo llamado `Elixir.Util.beam` en esa misma carpeta. Contiene la versión compilada del módulo.
5. Guarde los scripts de los siguientes ejemplos **en esa misma carpeta**, para que Elixir encuentre el módulo compilado al ejecutarlos.

Si más adelante modifica `util.exs`, recuerde volver a compilarlo con `elixirc` para que los cambios tengan efecto.
...
Con cuatro notas que promedian por encima de 3.0:

```
Ingrese la nota 1: 4.0
Ingrese la nota 2: 4.5
Ingrese la nota 3: 3.0
Ingrese la nota 4: 3.5
El estudiante aprobó el curso con una nota de 3.75.
```

Y con notas más bajas:

```
Ingrese la nota 1: 2.0
Ingrese la nota 2: 1.5
Ingrese la nota 3: 3.0
Ingrese la nota 4: 2.5
El estudiante no aprobó el curso con una nota de 2.25.
```

Las funciones `calcular_nota_curso_1/4`, `calcular_nota_curso_2/4` y `aprobado?/1` son lo suficientemente simples como para aprovechar la sintaxis `do:` de una sola línea. El `?` final de `aprobado?/1` sigue la convención de Elixir para las funciones que devuelven `true` o `false`.

> ⚠️ **Nota:** La función `calcular_nota_curso_2/4` se define desde ahora porque se utilizará en la Versión 4, pero como todavía no se invoca, Elixir mostrará al ejecutar la advertencia `function calcular_nota_curso_2/4 is unused`. Puede ignorarla por el momento.

### Versión 2

Para esta versión se ha usado el **pattern matching** para simplificar la generación de mensajes.

En el archivo de la Versión 1, **borre por completo la función `generar_mensaje/2`** (la que usa `if`/`else`) y escriba en su lugar estas dos líneas. El resto del módulo `Notas` no cambia.
...
Ahora hay dos funciones `generar_mensaje/2`, la primera asume que el estudiante aprobó el curso, mientras que la segunda asume que no lo aprobó, observe que el primer patrón coincide con `true` y el segundo con `false`.
...
Un cliente está indeciso entre dos planes de telefonía móvil y quiere saber cuál le sale más barato. Cada plan cobra un **cargo fijo mensual** más un **valor por cada giga** que consuma.

El programa debe solicitar el cargo fijo y el valor por giga de cada plan, junto con los gigas que el cliente consume al mes, calcular lo que pagaría en cada uno y mostrar un único mensaje:
...
- La función que compara los dos costos **no debe generar el mensaje**, sino retornar un átomo (`:plan_1`, `:plan_2` o `:empate`). Así la decisión queda separada del texto que se muestra.
...
- Con valores reales de un plan los costos son números grandes y aparecerán en notación científica. Aplique lo visto en las versiones opcionales de los ejemplos para mostrarlos con dos decimales.

---

## Para la próxima clase

- Repasar las estructuras `if`, `cond` y `case` vistas en esta guía, junto con los *guards*, ya que serán la base de la próxima clase.
- Las tuplas ya se usaron en esta guía para recibir el resultado de `Integer.parse/1`. Investigar ahora por qué en Elixir es una **convención** que las funciones devuelvan `{:ok, valor}` cuando tienen éxito y `{:error, motivo}` cuando fallan, y qué ventaja tiene frente a lanzar una excepción.
- Investigar qué es un **atributo de módulo** (por ejemplo `@nombre valor`) y en qué se diferencia de una variable.
- Investigar qué hace la estructura `with` en Elixir.
