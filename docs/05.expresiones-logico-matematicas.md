...
La estructura `unless` es la **negación de `if`**: ejecuta su bloque **solo cuando la condición es falsa**. Es útil para expresar de forma más legible la idea de "a menos que...".

```elixir
numero = 7

unless rem(numero, 2) == 0 do
  IO.puts("El número es impar")
end
```

El bloque se ejecuta porque `rem(7, 2) == 0` es `false`. `unless` también admite `else`:

```elixir
unless saldo >= 0 do
  IO.puts("Saldo en rojo")
else
  IO.puts("Saldo positivo")
end
```
...
La estructura `with` permite **encadenar varias operaciones que pueden fallar**, ejecutando el cuerpo solo si **todas** las coincidencias de patrón tienen éxito. Es ideal para trabajar con funciones que retornan tuplas como `{:ok, valor}` o `{numero, resto}`, evitando anidar múltiples `case`.

Considere el caso en que necesitamos leer dos números y sumarlos, pero cada lectura puede fallar:

```elixir
defmodule Suma do
  def main do
    resultado =
      with {n1, _} <- leer("Ingrese el primer número: "),
           {n2, _} <- leer("Ingrese el segundo número: ") do
        {:ok, n1 + n2}
      end

    case resultado do
      {:ok, suma} -> IO.puts("La suma es: #{suma}")
      :error -> IO.puts("Error: uno o ambos números no son válidos")
    end
  end

  defp leer(mensaje) do
    IO.gets(mensaje) |> String.trim() |> Integer.parse()
  end
end
...
- Cada línea usa el operador `<-`, que intenta hacer **pattern matching** entre el patrón de la izquierda y el resultado de la derecha.
- Si **todos** los patrones coinciden, se ejecuta el bloque `do`.
- Si **alguno no coincide** (por ejemplo, `Integer.parse/1` devuelve `:error` en lugar de `{numero, resto}`), `with` **detiene la ejecución** y devuelve ese valor que no coincidió (en este caso `:error`), **sin ejecutar el cuerpo**.

Esto evita el anidamiento profundo de varios `case` o `if`, haciendo el código más lineal y legible.

### Cláusula `else` en `with`

Se puede agregar un bloque `else` para manejar de forma explícita los valores que no coincidieron:

```elixir
with {n1, _} <- leer("Primer número: "),
     {n2, _} <- leer("Segundo número: ") do
  IO.puts("Suma: #{n1 + n2}")
else
  :error -> IO.puts("Entrada inválida")
end
```
...
El **pattern matching** (coincidencia de patrones) es uno de los mecanismos más importantes de Elixir y aparece en casi todas las construcciones del lenguaje. Hasta ahora lo hemos usado en `case` y al desestructurar tuplas; aquí lo veremos con más detalle.

### El operador `=` es un operador de coincidencia

En Elixir, `=` **no es una asignación** como en otros lenguajes, sino un **operador de coincidencia**. Cuando escribimos:

```elixir
x = 1
```

Elixir intenta hacer coincidir el patrón de la izquierda (`x`) con el valor de la derecha (`1`). Como `x` es una variable sin valor, se **enlaza** (*bind*) al valor `1`.

Esto explica por qué lo siguiente funciona:
...
[primero | resto] = [1, 2, 3]
IO.puts(primero)   # 1
IO.inspect(resto)  # [2, 3]
```

Y por qué esto **falla** con un `MatchError`, ya que las estructuras no tienen la misma forma:

```elixir
{a, b} = {1, 2, 3}  # ** (MatchError)
```

También podemos usar valores concretos como parte del patrón para validar la forma del dato:

```elixir
{:ok, valor} = {:ok, 42}          # coincide, valor = 42
{:ok, valor} = {:error, "falló"}  # ** (MatchError)
```

### El operador pin `^`

Como `=` enlaza variables, ¿qué pasa si queremos **comparar contra el valor actual** de una variable en lugar de reasignarla? Para eso existe el **operador pin** (`^`).

Sin el pin, la variable se vuelve a enlazar:

```elixir
x = 1
x = 2     # x simplemente se re-enlaza a 2
```

Con el pin, usamos el valor existente de `x` como parte del patrón:

```elixir
x = 1
^x = 1    # coincide: compara contra el valor actual de x (1)
^x = 2    # ** (MatchError): 2 no coincide con el valor de x (1)
```

El pin es especialmente útil dentro de `case` o de funciones cuando queremos que un patrón coincida con el valor de una variable ya conocida:

```elixir
esperado = :ok

case resultado do
  ^esperado -> IO.puts("Resultado esperado")
  otro -> IO.puts("Resultado inesperado: #{inspect(otro)}")
end
```

> ⚠️ **Importante:** Sin `^`, una variable en el lado izquierdo de un patrón siempre se **re-enlaza**; con `^`, se usa su **valor actual** para comparar.
