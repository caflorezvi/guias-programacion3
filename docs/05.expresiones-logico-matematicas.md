...
Las funciones `Integer.parse/1` y `Float.parse/1` retornan una **tupla** con dos elementos: el valor numérico convertido y la parte restante de la cadena. Su forma es:

```elixir
{numero, resto}
```

Si la cadena no comienza con un número válido, la función devuelve `:error`.

Para tratar estos posibles resultados, se utiliza un `case`. Esta estructura permite manejar de forma clara los distintos patrones que puede devolver una función, y es especialmente útil cuando se trabaja con tuplas o con valores que pueden fallar.

Observe que en el `case` se tienen dos patrones:

1. `{numero, _}`: Este patrón coincide con una tupla donde el primer elemento es el número convertido y el segundo elemento (representado por `_`) es ignorado. Si este patrón coincide, se devuelve `numero`.
2. `:error`: Este patrón coincide con el átomo `:error`. Si este patrón coincide, se imprime un mensaje de error y se devuelve un valor predeterminado (0 o 0.0).

Esto es lo que se conoce como **pattern matching** o coincidencia de patrones, una característica poderosa de Elixir que permite descomponer y analizar datos de manera concisa y clara.

#### Pattern Matching

El **pattern matching** es una de las características más potentes de Elixir. Permite verificar la forma de un dato y, al mismo tiempo, extraer valores de él. En este caso, se utiliza para:

* Reconocer la tupla `{numero, resto}`
* Detectar el átomo `:error`
* Elegir la acción correcta según el patrón recibido

Esto hace que el código sea más declarativo y menos propenso a errores, ya que cada caso se define explícitamente.
...
Finalmente, se modificará el módulo `Util` para que las funciones de lectura de enteros y flotantes utilicen una función auxiliar encargada de la captura del valor, su parseo y el manejo de errores. Con este cambio se **reduce la duplicación de código**.
...
### 🛠️ Reutilización de módulo `Util`
...
Aunque el código funciona, se puede mejorar utilizando `cond` o `guard` para simplificar el código y hacerlo más legible.
...
defmodule Loteria do
  ...
  def main do
    Util.leer("Ingrese un número entre #{@minimo} y #{@maximo}: ", :integer)
    |> validar_y_jugar()
    |> Util.imprimir_mensaje()
  end
