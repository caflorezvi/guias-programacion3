...
Observe que la última etapa usa la función `then/2`. ¿Por qué no basta con el pipe? Porque la comparación necesita la cadena **dos veces**: `cadena == String.reverse(cadena)`. El operador `|>` solo puede pasar el valor como primer argumento de una función, así que no permite usarlo en ambos lados de la comparación.
...
### 3. Evitar abreviaturas y siglas poco conocidas

Dado que el código puede ser leído por otros desarrolladores (o incluso por uno mismo en el futuro), es importante evitar abreviaturas y siglas que no sean ampliamente conocidas o que puedan generar confusión.
...
def calc_prom_nts(nts)
def val_email(e)
```

### 4. Consistencia en la nomenclatura

Mantener la **consistencia en la nomenclatura** a lo largo del proyecto. Si se elige un estilo para nombrar funciones y módulos, es importante seguirlo de manera uniforme.
...
### 5. Usar nombres en inglés

En contextos académicos o empresas locales, usar español puede ser aceptable. Lo importante es la consistencia en todo el proyecto.
...
# Aceptable en contextos locales - Español
defmodule AutenticacionUsuario do
  def validar_credenciales(usuario, contrasena)
end
```
### 6. Documentación adecuada

Como se mencionó anteriormente, es fundamental documentar adecuadamente los módulos y funciones para facilitar su comprensión y mantenimiento.

### 7. Delegar responsabilidades a funciones pequeñas
