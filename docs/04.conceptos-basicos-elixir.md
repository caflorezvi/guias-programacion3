...
Observe la función `generar_mensaje/1`: usa la estructura **`if`/`else`** para decidir qué texto retornar según el valor booleano que recibe. Su comportamiento es el mismo que en otros lenguajes —evalúa una condición y ejecuta una de dos ramas—, pero con dos diferencias importantes en Elixir:
