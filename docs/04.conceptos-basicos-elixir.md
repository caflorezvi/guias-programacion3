...
La sala de cine de tu ciudad ha implementado un nuevo sistema digital para la venta de boletas. El encargado de la taquilla desea que, al iniciar el sistema, se muestre un mensaje de bienvenida a los clientes.

El mensaje debe ser: `Bienvenidos al sistema de la Sala de Cine Central`.
...
Cree un nuevo archivo llamado `cine.exs` dentro de Visual Studio Code y escriba el siguiente código:

```elixir
IO.puts("Bienvenidos al sistema de la Sala de Cine Central")
```

Ejecute el programa usando el comando `elixir cine.exs` en la terminal. Debería ver el mensaje de bienvenida en la salida.
...
Modifique el archivo `cine.exs` (o créelo si no lo tiene) y escriba el siguiente código:
...
El módulo `Cine` contiene la función `main`, que es la que se invoca para mostrar el mensaje de bienvenida. Esto permite una mejor organización del código. Tenga en cuenta que es **necesario invocar** la función `main()` para que el **programa se ejecute**, por lo que se debe incluir la llamada a `Cine.main()` al **final del archivo**.
...
Modifique el archivo `cine.exs` o cree uno nuevo y escriba el siguiente código:
...
defmodule Cine do
  ...
  defp saludar do
    "Bienvenidos al sistema de la Sala de Cine Central"
  end
...
En este caso, la función `saludar` se define como privada (usando `defp`), lo que significa que solo puede ser llamada desde dentro del módulo `Cine`. Además, se ha separado la lógica de impresión en una función diferente (`imprimir_mensaje`). Esto permite una mejor organización del código y facilita la reutilización de funciones en otros contextos. 
...
La función `saludar` ahora acepta un parámetro `nombre`, que se utiliza para personalizar el mensaje de bienvenida. Así el programa se adapta a cualquier usuario. El nombre se pasa como argumento al llamar a `saludar` desde `main` y se **interpola** en el mensaje de bienvenida.
