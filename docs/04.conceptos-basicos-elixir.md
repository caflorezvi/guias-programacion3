...
Ejecute el programa usando el comando `elixir biblioteca.exs` en la terminal. Debería ver el mensaje de bienvenida en la salida.
...
Modifique el archivo `biblioteca.exs` (o créelo si no lo tiene) y escriba el siguiente código:
...
```elixir
def promedio(notas) do
  resultado = Enum.sum(notas) / length(notas)
  IO.puts("El promedio es: #{resultado}")
end
```

Reescríbala dentro de un módulo, separándola en **dos funciones**: una completamente pura, que solo calcule el promedio y lo devuelva, y otra que se encargue únicamente de imprimir el resultado. Agregue una función `main` que las coordine.
...
- Investigar qué **estructuras de decisión** ofrece Elixir (`if`, `cond`, `case`) y en qué se diferencian del `if`/`else` de un lenguaje como Java.
