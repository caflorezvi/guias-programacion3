...
flowchart TD
    %% Enum pipeline
    A1["Lista original: 1,2,3,4,5"] --> B1["filter > 2"]
    B1 --> C1["Lista filtrada: 3,4,5"]
    C1 --> D1["map * 2"]
    D1 --> E1["Lista transformada: 6,8,10"]
    E1 --> F1["reduce +"]
    F1 --> G1["Resultado final: 24"]

    %% Stream pipeline
    A2["Lista original: 1,2,3,4,5"] --> B2["Stream.filter > 2"]
    B2 --> C2["Stream.map * 2"]
    C2 --> D2["Enum.reduce +"]
...
- Secuencias infinitas → se generan bajo demanda y se procesan parcialmente.
- Pipelines complejos → cada elemento pasa una sola vez, sin listas intermedias.
