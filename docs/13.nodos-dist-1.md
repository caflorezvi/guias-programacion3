...
El formato del nombre de nodo es el siguiente:

![Diagrama de nombres de nodos](res/anatomia_nombres_nodos_short_long.svg)

A nivel de código, el nombre del nodo se representa de la siguiente manera:
...
Vamos a crear dos nodos en la misma máquina y establecer una conexión entre ambos. La idea es tener un nodo que envía un mensaje y otro que lo recibe. La diferencia respecto a los ejemplos anteriores es que ahora los procesos se pueden ejecutar de manera distribuida, es decir, cada nodo puede estar en una máquina diferente.

### Topología de Nodos

A nivel alto, la comunicación entre los dos nodos se puede representar de la siguiente manera:

![Diagrama de comunicación entre nodos](res/comparacion_send_local_vs_remoto_con_mailbox_v4.svg)
...
Como ambos nodos estarán en la misma máquina, se usa `--sname`. La cookie es opcional, pero conviene establecerla desde ya para que solo los nodos autorizados puedan comunicarse.
...
**Primero**, verifique que `epmd` esté corriendo con `epmd -names` y, si no lo está, inícielo con `epmd -daemon`.
...
- Implemente un mecanismo de *shutdown* con mensajes especiales como `:shutdown`. El mecanismo shutdown se puede implementar enviando un mensaje especial al proceso receptor que haga que este salga del loop y termine.

---

## Problemas comunes y soluciones

Casi todos los fallos al conectar nodos caen en uno de estos casos. La columna de la izquierda es lo que usted ve en pantalla.

| Síntoma | Causa | Solución |
| --- | --- | --- |
| Los nodos no se conectan aunque están en la misma red | Las cookies no coinciden | Consulte la de cada nodo con `Node.get_cookie()` y establezca la misma en todos |
| `{:error, {:already_started, ...}}` al iniciar un nodo | Ya existe un nodo con ese nombre en la máquina | Cierre el nodo existente o use un nombre distinto. Verifique los activos con `epmd -names` |
| `Protocol 'inet_tcp': register/listen error: econnrefused` | El demonio EPMD no está activo | Inícielo con `epmd -daemon` y verifique con `epmd -names` |
| Los nodos conectan en `localhost` pero no entre máquinas | El firewall bloquea los puertos | Abra el 4369 y un rango dinámico. En Linux, `sudo ufw allow 4369/tcp` y `sudo ufw allow 9000:9100/tcp` |
| `Node.connect/1` devuelve `false` o falla la resolución | El sistema no resuelve el nombre del host | Use la dirección IP, como `Node.connect(:"nodo@192.168.1.100")`, o agregue el host al archivo `hosts`, que está en `/etc/hosts` en Linux y macOS y en `C:\Windows\System32\drivers\etc\hosts` en Windows |
| Los nodos no se conectan y la configuración parece correcta | Un nodo usa shortnames y el otro longnames | Unifique el tipo de nombre en todo el clúster |
| Error al enviar mensajes porque el proceso receptor no existe | El proceso no se registró o ya finalizó | Confirme el registro con `Process.whereis(:nombre_proceso)` |
| El cliente no recibe respuesta del servidor | El servidor está ocupado, no escucha, o la red es lenta | Agregue un bloque `after` al `receive` para no quedar bloqueado, y verifique que el servidor esté activo |

Las cuatro primeras filas se evitan siguiendo la sección de *Requisitos previos* antes de levantar los nodos.

### Verificación rápida de conectividad

Estos comandos ayudan a diagnosticar cualquiera de los casos anteriores:
...
- Simular una tarea costosa que tome 5 segundos en completarse (por ejemplo, usando `:timer.sleep/1`).
...
### Diagrama de la comunicación entre nodos

La comunicación entre los nodos clientes y el nodo servidor se puede representar de la siguiente manera:

![Diagrama de comunicación entre nodos](res/multiples_clientes_concurrentes_servidor.svg)

Para implementar esta aplicación, siga los siguientes pasos:
...
defmodule Servidor do
  ...
  def loop do
    ...
    receive do
      ...
      {:tarea_costosa, pid} ->
        IO.puts("[#{timestamp()}] Petición: tarea_costosa (5s)")
  ...
  defp hacer_tarea_costosa(), do: :timer.sleep(5000)
...
> ⚠️ **Importante:** todo mensaje que el servidor pueda enviar necesita su propia cláusula en el `receive` del cliente. Si falta alguna, el mensaje **no se pierde**. Se queda esperando en el *mailbox* sin que nadie lo consuma, el `receive` sigue bloqueado y el cliente termina cayendo en el `after @timeout`. Es un error difícil de diagnosticar, porque el síntoma (un tiempo de espera agotado) sugiere que el servidor no respondió, cuando en realidad la respuesta llegó y nadie la reclamó.

### 3. Ejecutar y probar la aplicación

Primero, asegúrese otra vez de que `epmd` esté corriendo. Luego, en una terminal, ejecute el servidor:
...
Realice el siguiente ejercicio para practicar los conceptos aprendidos sobre aplicaciones distribuidas en Elixir.
