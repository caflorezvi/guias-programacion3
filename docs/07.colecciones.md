...
Elixir cuenta con cuatro colecciones principales: **listas, tuplas, mapas y keyword lists**. Todas son **inmutables**, por lo que cualquier operación que parezca modificar una colección en realidad produce una nueva colección, mientras que la original permanece intacta. Además, pueden contener datos de diferentes tipos y pueden desestructurarse mediante **Pattern Matching**. Para realizar operaciones de transformación y procesamiento sobre estas colecciones se utilizará el módulo `Enum`, que se estudiará más adelante.
...
Las listas se definen utilizando corchetes `[]` y los elementos se separan por comas. La función `length/1` devuelve cuántos elementos tiene una lista:
...
Para agregar elementos hay dos formas, y ambas devuelven una lista nueva:

```elixir
lista = [1, 2, 3]

[0 | lista]      # [0, 1, 2, 3]    -- agregar al inicio
lista ++ [4, 5]  # [1, 2, 3, 4, 5] -- concatenar al final
```

No cuestan lo mismo. Agregar al inicio es inmediato, porque la lista nueva reutiliza la anterior sin copiarla, mientras que `++` debe recorrer y copiar toda la lista de la izquierda. Con listas grandes conviene agregar por el inicio.
...
La clave no tiene que ser un átomo. Puede ser cualquier valor, lo que permite usar un identificador descriptivo como la cédula de una persona. Para obtener el valor de una clave se escribe la clave entre corchetes:
...
personas["123456789"] # %{nombre: "Ana", edad: 28}
```
...
Una **keyword list** (lista de palabras clave) es una lista de tuplas de dos elementos donde el primero es siempre un **átomo**, que hace de clave. Elixir ofrece una forma abreviada para escribirlas:
...
A diferencia de los mapas, las keyword lists **conservan el orden** en que se escriben y **permiten claves repetidas**.

Se usan principalmente para **pasar opciones a funciones**, y ya se han usado sin notarlo. En `IO.inspect(lista, label: "Cola")`, el `label: "Cola"` es una keyword list. Cuando es el último argumento de una función se pueden omitir los corchetes, por eso se escribe así y no `IO.inspect(lista, [label: "Cola"])`.
...
`Enum` tiene muchas más funciones (`count/1`, `sort_by/3`, `group_by/3`, `any?/2`, `find/2`, etc.). Las que hagan falta se irán introduciendo en los ejemplos; **no es necesario memorizarlas**, basta con saber que existen y buscarlas en la [documentación oficial de Enum](https://hexdocs.pm/elixir/Enum.html).
...
Aquí se usa `Enum` para procesar la colección de notas y tuplas para devolver múltiples valores desde una función. La primera cláusula de `calcular_promedio/1` usa **pattern matching** sobre la lista vacía para evitar una división por cero.
...
Este cambio obliga a ajustar todas las funciones, porque ahora cada elemento no es un número sino un mapa del que hay que **extraer** la nota. Cuando las claves son átomos, el valor se obtiene con un punto seguido del nombre de la clave. Si `estudiante` es `%{nombre: "pepe", nota: 3}`, entonces `estudiante.nota` devuelve `3` y `estudiante.nombre` devuelve `"pepe"`:
...
`Enum.each/2`, a diferencia de `Enum.map/2`, no construye una colección nueva sino que ejecuta una acción por cada elemento. Es la función indicada cuando lo único que interesa es el efecto secundario, en este caso imprimir.
