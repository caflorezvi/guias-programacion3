...
> ⚠️ **Importante:** este ejemplo utiliza el módulo `Util` construido previamente. Asegúrese de tener el archivo `util.exs` compilado con `elixirc` en la misma carpeta donde ejecute el script. Si prefiere no usarlo, reemplace las llamadas a `Util.imprimir_mensaje/1` por `IO.puts/1`.
