...
En este apartado, exploraremos las colecciones principales en Elixir: Listas, Tuplas, Mapas y Keyword Lists. Veremos cómo se definen, sus características, ejemplos de uso y cuándo conviene usar cada una.
...
IO.inspect(cola, label: "Cola") # [2, 3, 4, 5]
...
IO.inspect(resto, label: "Resto") # [30, 40]
...
nueva = [0 | lista]  
IO.inspect(nueva, label: "Nueva lista") #  [0, 1, 2, 3]

# Concatenar dos listas (crea una nueva lista)
otra = lista ++ [4, 5]  
IO.inspect(otra, label: "Otra lista") # [1, 2, 3, 4, 5]

# Quitar elementos (también crea una nueva lista)
menos = lista -- [2]   
IO.inspect(menos, label: "Lista sin elementos") # [1, 3]

# La lista original sigue igual
IO.inspect(lista, label: "Lista original") # [1, 2, 3]
```
...
IO.puts("Nombre: #{nombre}") # Nombre: Carlos
...
También es posible hacer **Pattern Matching en los parámetros de una función**. Como una función con nombre (`def`) debe ir siempre dentro de un módulo, la definimos así:
...
IO.puts("¿Es Carlos mayor de edad? #{Persona.es_mayor_de_edad?(usuario)}") # ¿Es Carlos mayor de edad? true
IO.puts("¿Es menor de edad? #{Persona.es_mayor_de_edad?(%{edad: 16})}")    # ¿Es menor de edad? false
```
...
IO.inspect(mayor, label: "Mapa actualizado") # %{nombre: "Carlos", edad: 31}

# El mapa original sigue igual
IO.inspect(usuario, label: "Mapa original") # %{nombre: "Carlos", edad: 30}
```
...
Una **keyword list** (lista de palabras clave) es un caso especial de lista: una lista de tuplas de dos elementos donde el primer elemento es siempre un **átomo** (la clave). Elixir ofrece una sintaxis abreviada para escribirlas:
...
A diferencia de los mapas, las keyword lists:

1. **Mantienen el orden** de los elementos tal como se insertaron.
2. **Permiten claves repetidas** (la misma clave puede aparecer varias veces).
3. **Solo admiten átomos como claves**.
...
# Lo mismo aplica a cualquier función que reciba opciones como último argumento
String.split("a-b-c", "-", parts: 2)  # => ["a", "b-c"]
```

Para más información, consulte la [documentación de Keyword](https://hexdocs.pm/elixir/Keyword.html).
...
- **Keyword lists**: Úselas para pasar opciones a funciones, o cuando necesite mantener el orden de inserción y permitir claves repetidas.
...
El módulo `Enum` es más general y se puede utilizar con **cualquier colección enumerable**, no solo listas. Es el módulo que más se usa para trabajar con colecciones. Estas son sus funciones fundamentales:

- `Enum.map/2`: transforma cada elemento y devuelve una nueva colección.
- `Enum.filter/2`: conserva los elementos que cumplen un predicado.
- `Enum.reduce/3`: combina todos los elementos en un único valor acumulado.
- `Enum.each/2`: ejecuta una función por cada elemento por sus efectos (no devuelve un resultado útil).
- `Enum.count/2`: cuenta los elementos que cumplen una condición.
- `Enum.sort_by/2`: ordena según el valor que devuelve una función.
- `Enum.group_by/2`: agrupa los elementos en un mapa según una función.

`Enum` tiene **muchas más** funciones (`uniq/1`, `any?/2`, `all?/2`, `find/2`, `frequencies/1`, `take/2`, `chunk_every/2`, `zip/2`, etc.). No es necesario memorizarlas: conviene dominar las fundamentales y consultar el resto en la [documentación oficial](https://hexdocs.pm/elixir/Enum.html) según se necesiten. En el **Ejemplo 2** veremos algunas de ellas en acción.
...
El `acc` puede ser de cualquier tipo de dato, no solo numérico. Por ejemplo, podríamos usar `reduce` para concatenar una lista de cadenas de texto o para construir un mapa a partir de una lista de tuplas, entre muchas otras posibilidades.

## Comprehensions (`for`)

Una **comprehension** es una forma concisa y declarativa de recorrer una o más colecciones, filtrar elementos y transformar los resultados, todo en una sola expresión. Se construye con la palabra clave `for`.

La sintaxis básica es: `for elemento <- coleccion, do: expresion`

```elixir
# Generar los cuadrados de una lista
for n <- [1, 2, 3, 4], do: n * n
# => [1, 4, 9, 16]
```

La parte `n <- [1, 2, 3, 4]` se llama **generador**: toma cada elemento de la colección. Se pueden agregar **filtros** (condiciones) para quedarse solo con ciertos elementos:

```elixir
# Solo los cuadrados de los números pares
for n <- [1, 2, 3, 4, 5, 6], rem(n, 2) == 0, do: n * n
# => [4, 16, 36]
```

También se pueden usar **varios generadores**, lo que produce todas las combinaciones posibles:

```elixir
# Todas las combinaciones de letras y números
for letra <- ["a", "b"], numero <- [1, 2], do: "#{letra}#{numero}"
# => ["a1", "a2", "b1", "b2"]
```

Para más información, consulte la [documentación de comprehensions](https://hexdocs.pm/elixir/comprehensions.html).

---

## Ejemplo 1

> ⚠️ **Importante:** este ejemplo utiliza el módulo `Util` construido en la guía 05. Asegúrese de tener el archivo `util.exs` compilado con `elixirc` en la misma carpeta donde ejecute el script. Si prefiere no usarlo, reemplace las llamadas a `Util.imprimir_mensaje/1` por `IO.puts/1`.
...
> **⚠️ Investigar** cómo mejorar la función `contar_segun_promedio/2` para que no tenga que recorrer la lista dos veces. Que halle la cantidad de estudiantes por encima y por debajo del promedio en una sola pasada. **Pista:** use `Enum.reduce/3` con una tupla `{menores, mayores}` como acumulador.
...
Se requiere una nueva función que devuelva la lista de estudiantes como una tupla que contenga el nombre del estudiante y su estado (`:aprobado` o `:reprobado`). Las siguientes funciones son fragmentos que se deben agregar al módulo `Estudiantes` de las versiones anteriores.
...
```elixir
    ...
    # => [{"pepe", :aprobado}, {"maria", :aprobado}, {"juan", :aprobado},
    #     {"luisa", :reprobado}, {"carlos", :reprobado}, {"ana", :aprobado},
    #     {"marcos", :reprobado}]
```
...
Una tienda registra las ventas del día en una lista de mapas. Cada venta indica el producto, su categoría y la cantidad vendida. Se necesitan algunos reportes que requieren funciones de `Enum` **distintas a las del ejemplo anterior**.
...
defmodule Ventas do
  def main do
    ventas = [
      ...
      %{producto: "Jugo", categoria: "Bebidas", cantidad: 4},
      %{producto: "Torta", categoria: "Panadería", cantidad: 1}
    ]

    IO.inspect(obtener_categorias_unicas(ventas), label: "Categorías")
    IO.inspect(agrupar_productos_por_categoria(ventas), label: "Productos por categoría")
    IO.inspect(contar_ventas_por_categoria(ventas), label: "Ventas por categoría")
    IO.inspect(sumar_unidades_por_categoria(ventas), label: "Unidades por categoría")

    IO.puts("¿Alguna venta de más de 4 unidades? #{hay_venta_alta?(ventas)}")
    IO.puts("¿Todas las ventas son válidas? #{ventas_validas?(ventas)}")

    IO.inspect(buscar_primera_venta_categoria(ventas, "Bebidas"), label: "Primera venta de Bebidas")
  end

  # Categorías sin repetir (Enum.uniq)
  ...
  # Total de unidades vendidas por categoría (Enum.reduce construyendo un mapa)
  defp sumar_unidades_por_categoria(ventas) do
    Enum.reduce(ventas, %{}, fn venta, acc ->
      Map.update(acc, venta.categoria, venta.cantidad, fn total -> total + venta.cantidad end)
    end)
  end
  ...
  # Primera venta de una categoría dada (Enum.find)
...
En este ejemplo aparecen funciones que no usamos en el Ejemplo 1 y que resultan muy útiles para generar reportes:

- **`Enum.uniq/1`**: elimina los elementos repetidos conservando el orden.
- **`Enum.group_by/3`**: agrupa los elementos en un mapa; el tercer argumento permite transformar lo que se guarda en cada grupo (aquí, solo el nombre del producto).
- **`Enum.frequencies_by/2`**: cuenta cuántos elementos hay por cada clave calculada.
- **`Enum.reduce/3` junto con `Map.update/4`**: patrón habitual para acumular resultados dentro de un mapa (aquí, sumar las cantidades de cada categoría).
- **`Enum.any?/2` y `Enum.all?/2`**: verifican si *alguno* o *todos* los elementos cumplen una condición; devuelven un booleano.
- **`Enum.find/2`**: devuelve el primer elemento que cumple una condición (o `nil` si ninguno lo hace).

---

## Ejercicios propuestos

Resuelva los siguientes ejercicios utilizando las colecciones vistas en esta guía. Se recomienda usar el módulo `Enum` para procesar las colecciones de manera eficiente.
...
- Repasar la descomposición de listas con `[head | tail]` vista en esta guía, ya que será la base para procesar listas recursivamente.