...
Cada valor puede ser de cualquier tipo de dato, incluyendo listas, tuplas y otros mapas.
...
Esto permite usar como clave un identificador descriptivo, como la cédula de una persona o el ID de un producto. Por ejemplo, para guardar la información de quienes compran en una tienda usando su cédula como clave:
...
Conviene tener presente que **ninguna de estas funciones modifica la colección que recibe**: producen un valor nuevo y dejan intacto el original. Son, en su gran mayoría, **funciones puras** en el sentido que estudiamos en la guía de programación funcional, lo que hace que trabajar con colecciones sea predecible y seguro.
...
Aquí se usa `IO.inspect/1` y no `IO.puts/1` por la razón vista en la guía de programación funcional: la lista de tuplas no se puede convertir a texto.
...
Todas estas funciones son distintas a las del Ejemplo 1 y resultan muy útiles para generar reportes. Ejecute el programa y compare cada salida con la función que la produjo.
