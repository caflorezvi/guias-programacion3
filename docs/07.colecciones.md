...
Una **lista** es una colección ordenada de elementos, que puede contener cualquier tipo de dato, incluyendo otras listas. Las listas se definen utilizando corchetes `[]` y los elementos se separan por comas:

```elixir
mi_lista = [1, 2, 3, 4, 5]
```

La operación más característica de las listas es separarlas en **cabeza y cola** mediante Pattern Matching. Será la base para recorrerlas de forma recursiva en la siguiente guía:
...
IO.inspect(cola, label: "Cola") # Cola: [2, 3, 4, 5]

# También se pueden tomar varios elementos del inicio
[primero, segundo | resto] = [10, 20, 30, 40]
IO.puts("#{primero} y #{segundo}") # 10 y 20
IO.inspect(resto, label: "Resto")  # Resto: [30, 40]

# Intentar desestructurar una lista vacía producirá un error
# [cabeza | cola] = [] # ** (MatchError) no match of right hand side value: []
```

Esa misma sintaxis sirve para **agregar** un elemento al inicio, que es la forma más eficiente de crecer una lista: gracias al *structural sharing*, la nueva lista reutiliza la anterior como cola en lugar de copiarla.
...
Un **mapa** es una colección de **pares clave-valor**, donde cada clave es única. Los mapas se definen utilizando `%{}` y los pares se separan por comas:
...
Y, como ocurría con las tuplas, es posible hacer **Pattern Matching en los parámetros de una función**. Como una función con nombre (`def`) debe ir siempre dentro de un módulo, la definimos así:
...
También se pueden usar **varios generadores**, en cuyo caso se producen todas las combinaciones posibles:
...
defmodule Ventas do
  def main do
    ...
    Util.imprimir_mensaje("¿Alguna venta de más de 4 unidades? #{hay_venta_alta?(ventas)}")
    Util.imprimir_mensaje("¿Todas las ventas son válidas? #{ventas_validas?(ventas)}")

    IO.inspect(buscar_primera_venta_categoria(ventas, "Bebidas"), label: "Primera venta de Bebidas")
    IO.inspect(buscar_primera_venta_categoria(ventas, "Lácteos"), label: "Primera venta de Lácteos")
  end
