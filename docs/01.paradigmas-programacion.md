...
Un **paradigma de programación** representa una filosofía o forma de pensar para resolver problemas mediante programación. No es un lenguaje en sí mismo, sino un conjunto de ideas, principios y conceptos que influyen en cómo se estructura y se escribe el código.

Como analogía, el **paradigma** puede entenderse como la *estrategia* o *forma de viajar*: ya sea siguiendo instrucciones precisas paso a paso, o simplemente indicando el destino y dejando que el sistema determine cómo llegar. El **lenguaje de programación**, en cambio, es el *vehículo* que permite llevar a la práctica esa estrategia.

Por ejemplo:

- Viajar siguiendo instrucciones exactas corresponde al paradigma **imperativo**.
- Viajar indicando únicamente el destino final corresponde al paradigma **declarativo**.

En este sentido, el **lenguaje de programación** es la herramienta concreta que permite expresar una solución de acuerdo con la forma de viajar propuesta por el paradigma elegido.

## Principales Paradigmas de Programación

En el siguiente diagrama se resumen los principales paradigmas de programación y sus características clave:

![Diagrama de Paradigmas de Programación](res/paradigmas_programacion.svg)

Existen otros paradigmas además de los que estudiaremos (por ejemplo, el **lógico**, representado por Prolog, o el **reactivo**), pero en este curso nos concentraremos en la gran división entre el enfoque **imperativo** y el **declarativo/funcional**, que es la más relevante para entender Elixir.
...
Organiza el código en torno a "objetos", que son unidades que agrupan datos (atributos) y los comportamientos que operan sobre esos datos (métodos). En este curso la presentamos junto al enfoque imperativo porque, en la práctica, la mayoría del código orientado a objetos se escribe de forma imperativa (con estado mutable y pasos explícitos). No obstante, conviene tener presente que esta clasificación es una **simplificación**: hay autores que consideran la POO como un eje independiente que puede combinarse tanto con estilos imperativos como funcionales.
...
Veamos cómo sumar los números pares de una lista. Aunque está escrito en Java (un lenguaje orientado a objetos), el estilo del código es puramente **imperativo**: describe cada paso y modifica una variable `sumaPares` en cada iteración, sin apoyarse en encapsulamiento, herencia ni polimorfismo.
...
```
Aquí, el control del flujo y la gestión del estado son manuales y explícitos, se debe escribir cada paso detalladamente.
...
En la siguiente tabla se resumen las diferencias clave entre la Programación Imperativa/POO y la Programación Funcional:
...
Para la próxima clase, se propone trabajar sobre un problema sencillo (por ejemplo: calcular el promedio de notas, contar elementos que cumplen una condición o transformar una lista de datos) y analizarlo desde distintos paradigmas de programación.

Realice lo siguiente:

- Describir cómo resolverían el problema usando un enfoque imperativo.
- Describir cómo lo abordarían desde un enfoque declarativo/funcional.
- Identificar qué decisiones cambian en cada caso (uso de estado, control del flujo, nivel de detalle).
- Reflexionar brevemente sobre cuál enfoque les resulta más natural y por qué.

La actividad se realizará sin escribir código, enfocándose únicamente en la forma de pensar y razonar la solución en cada paradigma.
