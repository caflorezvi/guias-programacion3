...
Valim buscaba un lenguaje que combinara la alta productividad y la sintaxis elegante de Ruby con una solución robusta y nativa para la alta concurrencia y la tolerancia a fallos, algo que representaba un desafío en el ecosistema de Rails.

En lugar de crear todo desde cero, Valim optó por construir Elixir sobre la **plataforma de Erlang**, una base sólida y probada durante décadas. Así, la filosofía de Elixir se puede resumir en: *"La productividad y elegancia de Ruby sobre la potencia y robustez de Erlang"*.

## El superpoder secreto de Elixir: El ecosistema Erlang (BEAM y OTP)

El verdadero poder de Elixir no reside únicamente en su sintaxis, sino en la plataforma tecnológica sobre la que se ejecuta.
...
La BEAM es para Elixir lo que la JVM (Java Virtual Machine) es para Java o Scala: un entorno de ejecución de alto rendimiento especializado. Sin embargo, la BEAM fue construida originalmente para soportar sistemas de telecomunicaciones que debían permanecer activos 24/7 sin fallos.

### 2. La plataforma OTP (Open Telecom Platform)

OTP es un conjunto de principios, patrones y herramientas de Erlang para construir sistemas concurrentes, resilientes y tolerantes a fallos; en esencia, es el “manual de instrucciones” que Erlang y Elixir utilizan para organizar y manejar la concurrencia.
...
-   **Concurrente**: La concurrencia es un pilar fundamental del lenguaje, no una característica añadida. Crear un proceso para realizar una tarea en paralelo es una operación trivial y común, lo que facilita enormemente el desarrollo de aplicaciones que hacen muchas cosas a la vez.
...
- **Pinterest:** red social visual basada en imágenes y videos.
...
- Repasar las características principales de Elixir vistas en esta guía, ya que serán la base de la próxima clase sobre programación funcional.
- Investigar qué es una **función pura** y qué significa la **inmutabilidad** en programación funcional.
- Investigar a qué se refieren los **efectos secundarios** (*side effects*) en programación.
- Instalar Elixir y Erlang en su computador personal (si aún no lo ha hecho).