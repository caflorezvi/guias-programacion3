...
Ya lo usamos en el ejemplo del chat: cada vez que se creaba una sala nueva, se iniciaba un `GenServer` como hijo del `DynamicSupervisor` con `DynamicSupervisor.start_child/2`. Esto es ideal cuando no se sabe de antemano cuántos procesos existirán (salas de chat, partidas, conexiones de clientes, etc.).
