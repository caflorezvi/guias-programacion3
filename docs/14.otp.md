...
- `get_and_update/2`: Lee y modifica en una operación atómica
...
sequenceDiagram
    ...
    Note over Agent: Serialización garantizada (uno a la vez)
...
> **⚠️ Nota:** Como regla general, se recomienda usar `Agent` para casos simples. Si necesita más control o lógica compleja, migre a `GenServer`.
...
sequenceDiagram
    ...
    Note over Cliente,CB: CAST (Asíncrono)
