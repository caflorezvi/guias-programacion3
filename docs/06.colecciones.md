...
## Listas de palabras clave (Keyword Lists)

Una **keyword list** (lista de palabras clave) es una lista de tuplas de dos elementos donde el **primer elemento es un átomo** (la clave). Elixir ofrece una sintaxis especial para escribirlas de forma más cómoda:

```elixir
# Forma explícita (lista de tuplas)
opciones = [{:color, "rojo"}, {:tamano, 10}]

# Forma abreviada (equivalente)
opciones = [color: "rojo", tamano: 10]
```

Las keyword lists tienen tres características clave:

1. Las **claves son átomos**.
2. Las claves **están ordenadas** (mantienen el orden de definición).
3. Las claves **pueden repetirse** (a diferencia de los mapas).

Su uso más común es **pasar opciones a funciones**. De hecho, ya las hemos usado sin saberlo:

```elixir
IO.inspect([1, 2, 3], label: "Mi lista")   # label: "Mi lista" es una keyword list
Enum.sort_by(lista, &(&1.nota), :desc)
```

Cuando una keyword list es el **último argumento** de una función, se pueden omitir los corchetes:
...
Para acceder a sus valores se puede usar el módulo `Keyword`:

```elixir
opciones = [color: "rojo", tamano: 10]
Keyword.get(opciones, :color)              # "rojo"
Keyword.get(opciones, :forma, "ninguna")   # "ninguna" (valor por defecto)
```

> ⚠️ **¿Keyword list o mapa?** Use **keyword lists** para listas de opciones (donde el orden importa o puede haber claves repetidas) y **mapas** para datos estructurados donde necesita acceso rápido por clave única.

---

## Comprehensions (`for`)

Una **comprehension** es una forma concisa y declarativa de **recorrer una o más colecciones, filtrar elementos y construir una nueva colección**. Se escribe con la construcción `for`.

La forma más simple transforma cada elemento (similar a `Enum.map/2`):
...
El generador `n <- [1, 2, 3, 4]` toma cada elemento de la lista y lo enlaza a `n`.

### Filtros

Se pueden agregar condiciones (filtros) que descartan elementos:
...
Solo los elementos que cumplen **todos** los filtros llegan al cuerpo de la comprehension.

### Múltiples generadores

Se pueden combinar varias colecciones, produciendo todas las combinaciones posibles:
...
Por defecto una comprehension devuelve una lista, pero con `:into` se puede construir otra estructura, como un mapa:

```elixir
for {clave, valor} <- [a: 1, b: 2], into: %{} do
  {clave, valor * 10}
end
# => %{a: 10, b: 20}
```

> ⚠️ **Nota:** Las comprehensions son azúcar sintáctico muy legible para casos de *map + filter* combinados. Para transformaciones más complejas, las funciones de `Enum` y `Stream` siguen siendo la herramienta principal.
