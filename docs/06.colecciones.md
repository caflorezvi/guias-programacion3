...
Para más información sobre el módulo `List`, se puede consultar la [documentación oficial](https://hexdocs.pm/elixir/List.html), y para `Map` se puede consultar la [documentación oficial](https://hexdocs.pm/elixir/Map.html).
...
La forma más idiomática y recomendada en Elixir es usar **pattern matching** directamente en la firma de la función anónima para desestructurar el mapa del estudiante. Esto hace el código más claro y conciso:

```elixir
defp agregar_estado(lista) do
  Enum.map(lista, fn %{nombre: nombre, nota: nota} ->
    estado = if nota >= 3, do: :aprobado, else: :reprobado
    {nombre, estado}
  end)
end
```

Esta versión es preferible porque nombra explícitamente los campos que se extraen del mapa (`nombre` y `nota`), mejorando la legibilidad.

Aunque también es posible escribirlo de forma más compacta con el operador de captura `&`, la versión con `fn` y pattern matching suele ser más fácil de leer para otros desarrolladores:
