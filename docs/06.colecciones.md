...
> ⚠️ **Mapa vs. keyword list:** use un **mapa** cuando necesite asociar claves a valores con acceso eficiente y claves únicas; use una **keyword list** cuando necesite mantener el orden, permitir claves repetidas o pasar opciones a una función.
...
> **⚠️ Investigar** cómo mejorar la función `total_segun_promedio/2` para que no tenga que recorrer la lista dos veces. Que halle la cantidad de estudiantes por encima y por debajo del promedio en una sola pasada. **Pista:** use `Enum.reduce/3` con una tupla `{menores, mayores}` como acumulador.
...
defmodule Ventas do
  def main do
    ...
    IO.inspect(categorias_unicas(ventas), label: "Categorías")
    IO.inspect(productos_por_categoria(ventas), label: "Productos por categoría")
    IO.inspect(ventas_por_categoria(ventas), label: "Ventas por categoría")
    IO.inspect(unidades_por_categoria(ventas), label: "Unidades por categoría")
    ...
    IO.inspect(primera_venta_categoria(ventas, "Bebidas"), label: "Primera venta de Bebidas")
  end

  # Categorías sin repetir (Enum.uniq)
  defp categorias_unicas(ventas) do
    ventas
  ...
  defp productos_por_categoria(ventas) do
  ...
  defp unidades_por_categoria(ventas) do
  ...
  defp primera_venta_categoria(ventas, categoria) do
...
- Practique las **keyword lists** y los **comprehensions (`for`)** vistos en esta guía con sus propios ejemplos.
