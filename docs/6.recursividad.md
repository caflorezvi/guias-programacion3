...
Cada vez que una función se llama a sí misma, se crea una **nueva entrada** en la pila de llamadas. Cuando la función alcanza el caso base, las llamadas comienzan a resolverse en orden inverso, devolviendo los resultados a las llamadas anteriores. Este proceso es gestionado automáticamente por el entorno de ejecución de Elixir.

Tenga en cuenta que la pila de llamadas no es exclusiva de la recursividad; cuando una función llama a otra función, también se crea una nueva entrada en la pila de llamadas, y cuando la función llamada termina, el control regresa a la función que la llamó.

## Estado de las variables

En Elixir, las variables son inmutables. Cada vez que una función se llama a sí misma, se **crea un nuevo contexto de ejecución** con sus propias variables. Esto significa que los cambios en las variables dentro de una llamada recursiva no afectan a las variables en las llamadas anteriores. 

Si desea mantener el estado entre llamadas recursivas, debe pasar los valores necesarios como argumentos en la llamada recursiva.
...
La recursividad es una técnica común para procesar listas en Elixir. Podemos utilizarla para implementar funciones que operen sobre listas de manera elegante y concisa. Por ejemplo, podemos definir una función que sume todos los elementos de una lista.
...
La recursividad de cola es una forma de recursión en la que la llamada recursiva es la última operación que se realiza en la función. Esto permite que el compilador o el intérprete optimice la llamada y evite el crecimiento de la pila, lo que puede prevenir desbordamientos de pila en casos de recursión profunda.
...
Observe que se ha utilizado un parámetro `acumulado` para mantener los cálculos intermedios, luego, en el caso base, simplemente devolvemos el valor acumulado.
...
3. Devolver el menor elemento de una lista.
...
7. Contar el número de dígitos de un número. Se asume que el número es positivo. No puede convertir el número a `string`.
...
La recursividad no es solo un concepto de programación; también es un patrón que encontramos en la naturaleza. Por ejemplo, las ramas de un árbol se dividen en ramas más pequeñas que replican la forma del árbol, los ríos se ramifican en afluentes que a su vez se subdividen, y los bronquios de los pulmones se bifurcan en ramas cada vez más finas. En todos estos casos, un mismo patrón se repite a diferentes escalas.

Este tipo de patrones recursivos también se observa en estructuras matemáticas y **fractales**, donde una forma se repite a diferentes niveles de magnificación.
