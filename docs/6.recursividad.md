...
Gráficamente, el proceso se puede visualizar como una pila de llamadas así:

![Pila recursiva](media/recur.svg)
...
Se quiere calcular `Matematicas.potencia(2, 3)`. El paso a paso sería el siguiente:
...
Se quiere calcular `Matematicas.multiplicar(2, 3)`. El paso a paso sería el siguiente:
...
defmodule Cadenas do
  ...
  def invertir(cadena) do
    ...
    String.last(cadena) <> invertir(cadena_restante) # Concatenar el último carácter con la cadena restante invertida
  end
end
``` 
...
1. `invertir("hola")` = "a" <> `invertir("hol")`
2. `invertir("hol")` = "l" <> `invertir("ho")`
3. `invertir("ho")` = "o" <> `invertir("h")`
4. `invertir("h")` = "h" <> `invertir("")`
5. `invertir("")` = ""
...
1. `sumar([1, 2, 3])` = 1 + `sumar([2, 3])`
2. `sumar([2, 3])` = 2 + `sumar([3])`
3. `sumar([3])` = 3 + `sumar([])`
4. `sumar([])` = 0
...
1. `contar_pares([1, 2, 3, 4, 5])` = `contar_pares([2, 3, 4, 5])` (1 es impar, no se cuenta)
...
1. `maximo([3, 1, 4, 1, 5, 9])` = `maximo([1, 4, 1, 5, 9], 3)` (3 es el mayor inicial)
...
Se quiere calcular `Matematicas.factorial(5)`. El paso a paso sería el siguiente:

1. `factorial(5)` = `factorial(5, 1)` (se inicia el acumulado en 1)
...
Se quiere calcular `Matematicas.fibonacci(5)`. El paso a paso sería el siguiente:

1. `fibonacci(5)` = `fibonacci(5, 0, 1)` (se inician los dos primeros números de la secuencia 0 y 1)
...
Por lo tanto, `fibonacci(5)` = 5. En esta implementación no se recalculan valores intermedios, por lo que es más eficiente. Además, no se acumulan llamadas en la pila.
...
1. `sumar([1, 2, 3])` = `sumar([1, 2, 3], 0)` (se inicia el acumulado en 0)
2. `sumar([1, 2, 3], 0)` = `sumar([2, 3], 1)`
3. `sumar([2, 3], 1)` = `sumar([3], 3)`
4. `sumar([3], 3)` = `sumar([], 6)`
5. `sumar([], 6)` = 6

Por lo tanto, `Listas.sumar([1, 2, 3])` = 6. Las sumas intermedias se acumulan en el parámetro `acumulado`, y no hay operaciones pendientes después de la llamada recursiva.
