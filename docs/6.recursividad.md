...
1. `Listas.contar_pares([1, 2, 3, 4, 5])` = `Listas.contar_pares([2, 3, 4, 5])` (1 es impar, no se cuenta)
2. `Listas.contar_pares([2, 3, 4, 5])` = 1 + `Listas.contar_pares([3, 4, 5])` (2 es par, se cuenta)
3. `Listas.contar_pares([3, 4, 5])` = `Listas.contar_pares([4, 5])` (3 es impar, no se cuenta)
4. `Listas.contar_pares([4, 5])` = 1 + `Listas.contar_pares([5])` (4 es par, se cuenta)
5. `Listas.contar_pares([5])` = `Listas.contar_pares([])` (5 es impar, no se cuenta)
6. `Listas.contar_pares([])` = 0

Por lo tanto, `Listas.contar_pares([1, 2, 3, 4, 5])` = 0 + 1 + 0 + 1 + 0 = 2.
...
2. `maximo([1, 4, 1, 5, 9], 3)` = `maximo([4, 1, 5, 9], 4)` (4 es mayor que 3, se actualiza)
3. `maximo([4, 1, 5, 9], 4)` = `maximo([1, 5, 9], 4)` (no se actualiza, 1 es menor que 4)
4. `maximo([1, 5, 9], 4)` = `maximo([5, 9], 4)` (no se actualiza, 1 es menor que 4)
...
Lenguajes como **Elixir** y **Erlang** aplican esta optimización de forma automática. Para que una función se considere **recursiva de cola**, la llamada recursiva debe estar al final, sin que haya ninguna operación pendiente después de ella.
