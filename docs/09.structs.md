...
Los mapas admiten cualquier clave nueva; los structs no. Más adelante veremos en detalle cómo se define un struct con `defstruct`.
...
# Mapa: permite agregar nuevas claves
mapa = %{nombre: "Juan", edad: 30}

mapa = %{mapa | edad: 31}
mapa = Map.put(mapa, :telefono, "123")

# Struct: solo admite las claves definidas
usuario = %Usuario{nombre: "Juan", edad: 30}

usuario = %{usuario | edad: 31}  # Válido
usuario = %{usuario | telefono: "123"}  # Error: clave no definida
```
...
> ⚠️ **Importante:** la sintaxis `%{struct | campo: valor}` solo permite actualizar campos **que ya existen** en el struct. Si intenta `%{cancion | album: "A Night at the Opera"}`, obtendrá un error, porque `album` no está definido en `defstruct`.
