...
El módulo `Gimnasio` va a tener la lógica para manejar los socios. Se usará un mapa para almacenarlos, porque da acceso rápido a los datos mediante una clave única, la cédula del socio.
...
El uso de `=` aquí no solo es más corto. Al ser una coincidencia de patrones, si algún día `Socio.desinscribir_clase/2` empezara a retornar un error, el programa fallaría de inmediato con un `MatchError` señalando la línea exacta, en lugar de continuar con un valor inesperado.
...
> **⚠️ Sugerencia:** Igual que en el ejemplo del gimnasio, haga que cada función retorne tuplas `{:ok, ...}` o `{:error, razon}` para dejar los errores explícitos, y delegue en las funciones del módulo `Cuenta` la lógica propia de cada cuenta.
