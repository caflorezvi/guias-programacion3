...
El módulo `Gimnasio` va a tener la lógica para manejar los socios. En este caso, se usará un mapa para almacenarlos, ya que permite un acceso rápido a los datos. 

La clave será la cédula del socio y el valor será el struct `Socio`.
...
El acceso a un socio se hace mediante su cédula, que es la clave en el mapa. Esta operación es muy eficiente.
...
- Valor total del inventario (suma del precio por la cantidad de cada producto).
...
- Listado de productos con cantidad mayor a un valor dado.
- Listado de productos ordenados por precio (ascendente y descendente).
