...
La recursividad es una técnica de programación en la que **una función se llama a sí misma** para resolver un problema. En Elixir, la recursividad es una herramienta poderosa y se utiliza con frecuencia para trabajar con estructuras de datos y cadenas, así como para resolver problemas matemáticos y algorítmicos.
...
Observe que la función pública `maximo/1` incluye una cláusula para la **lista vacía**, que retorna `{:error, :lista_vacia}`. Sin ella, llamar a `maximo([])` provocaría un error, ya que ningún patrón coincidiría.
...
Por eso la recursión no es solo una técnica más: es la forma natural de expresar y recorrer este tipo de estructuras, y reconocer el patrón ayuda a identificar cuándo un problema se puede resolver recursivamente.
