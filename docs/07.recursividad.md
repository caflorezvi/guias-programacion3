...
Aunque esta implementación es simple y directa, no es la más eficiente debido a la recomputación de valores. Más adelante, exploraremos una versión optimizada usando recursividad de cola.

Por ejemplo, `fibonacci(40)`: ~102 millones de llamadas y `fibonacci(50)`: más de mil millones de llamadas. Su tiempo de ejecución crece exponencialmente. 
...
└─► contar_vocales("undo")    (M no es vocal)
    └─► 1 + contar_vocales("ndo")    (u es vocal)
            └─► contar_vocales("do")    (n no es vocal)
                └─► 1 + contar_vocales("o")    (d no es vocal)
...
└─► contar_pares([2, 3, 4, 5])                    (1 es impar, no cuenta)
    └─► 1 + contar_pares([3, 4, 5])               (2 es par, cuenta)
            └─► contar_pares([4, 5])              (3 es impar, no cuenta)
                └─► 1 + contar_pares([5])         (4 es par, cuenta)
                        └─► contar_pares([])      (5 es impar, no cuenta)
                            └─► 0
...
```
fibonacci(5, 0, 1)   → n=5, a=0, b=1
fibonacci(4, 1, 1)   → n=4, a=1, b=1
fibonacci(3, 1, 2)   → n=3, a=1, b=2
fibonacci(2, 2, 3)   → n=2, a=2, b=3
fibonacci(1, 3, 5)   → n=1, a=3, b=5
fibonacci(0, 5, 8)   → n=0, retorna 5
```
...
```
sumar([1, 2, 3], 0)   → sumar([2, 3], 1)
sumar([2, 3], 1)      → sumar([3], 3)
sumar([3], 3)         → sumar([], 6)
sumar([], 6)          → 6
``` 
...
2. **Ríos**: Río principal → afluentes → tributarios
