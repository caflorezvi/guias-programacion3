...
La respuesta de cada nodo se va resolviendo a medida que se llega a los casos base:
...
Aunque esta implementación es simple y directa, no es la más eficiente debido a la recomputación de valores (observe cómo `fibonacci(2)` o `fibonacci(3)` se calcula varias veces). Más adelante, exploraremos una versión optimizada usando recursividad de cola.
