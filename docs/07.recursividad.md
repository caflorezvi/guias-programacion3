...
Aunque esta implementación es simple y directa, no es la más eficiente debido a la recomputación de valores (observe cómo `fibonacci(2)` o `fibonacci(3)` se calcula varias veces). Por ejemplo, `fibonacci(40)` realiza aproximadamente 102 millones de llamadas y `fibonacci(50)` más de mil millones. Su tiempo de ejecución crece exponencialmente. 
