...
Para más información, puede consultar la [documentación oficial de Mix](https://hexdocs.pm/mix/Mix.html).
...
En las guías de **OTP** y **Supervisores** vimos `GenServer` y supervisores ejecutados desde scripts `.exs`. En un proyecto Mix real, lo habitual es que la función `start/2` de la `Application` **inicie un supervisor raíz**, que a su vez arranca y vigila los procesos de la aplicación. Así, todo el sistema queda supervisado desde el arranque.
...
Este patrón (una `Application` que inicia un árbol de supervisión) es la **base de toda aplicación OTP de producción** y es el que se espera en el proyecto final, donde un `DynamicSupervisor` gestiona las batallas creadas en tiempo de ejecución.
...
Realice los siguientes ejercicios para practicar el uso de Mix, la creación de proyectos, la organización del código y la escritura de pruebas unitarias.

### Ejercicio 1

Cree un nuevo proyecto Mix llamado `pokedex` e integre todo lo que hicimos previamente en el proyecto que era sobre el CRUD de Pokémon y sus movimientos. Asegúrese de que el proyecto compile y se ejecute correctamente. Además, agregue al menos dos pruebas unitarias para verificar el correcto funcionamiento de las funciones CRUD.
...
pokedex/
├── lib/
│   ├── pokedex.ex            # Módulo principal
│   ├── pokedex/
│   │   ├── pokemon.ex        # Gestión de Pokémon
│   │   ├── movimiento.ex     # Gestión de movimientos
│   │   └── archivo.ex        # Persistencia en CSV
└── test/
    ├── pokedex_test.exs
    └── pokedex/
        ├── pokemon_test.exs
...
Convierta el sistema de gestión de tareas de la guía de **OTP** en una aplicación OTP completa utilizando Mix. Cree un proyecto Mix, defina un árbol de supervisión para el servidor de tareas y asegúrese de que el servidor se inicie automáticamente cuando se inicie la aplicación.
