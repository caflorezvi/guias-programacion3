...
> ⚠️ **Importante:** La diferencia clave entre concurrencia y paralelismo radica en que la **concurrencia** se enfoca en la gestión eficiente de múltiples tareas, mientras que el **paralelismo** se centra en la ejecución simultánea de esas tareas.
...
* Por eso se puede escalar hasta **miles o incluso millones** de procesos concurrentes en una sola máquina, algo impensable con hilos tradicionales.
...
En este caso, es probable que el mensaje del proceso hijo no se imprima, ya que el **proceso principal termina antes de que el proceso hijo complete su tarea**. Básicamente, pasa lo siguiente:
...
A continuación se muestra un diagrama ilustrativo del mailbox: varios procesos envían mensajes que se acumulan en orden, y el actor receptor los consume uno a uno con pattern matching. Esto explica visualmente por qué no hay race conditions: nunca hay dos mensajes "siendo procesados" al mismo tiempo dentro del mismo actor.
...
- `Process.info/1` y `Process.info/2`: Obtienen información de un proceso. La versión `/1` devuelve todos los datos (estado, memoria, mensajes en la cola, etc.), mientras que la `/2` permite consultar un dato específico, como `Process.info(pid, :message_queue_len)`.
...
> **⚠️ Importante:** Aunque más adelante veremos ejemplos prácticos, es recomendable consultar la documentación oficial para explorar todas las funciones disponibles en el módulo `Process`.
...
A continuación, se presentan algunos ejemplos prácticos que ilustran el uso de procesos y concurrencia en Elixir utilizando tanto `spawn` como el módulo `Task`. Intente hacer su propia implementación de cada ejemplo para comprender mejor cómo funcionan los procesos en Elixir.

### Ejemplo 1

Imagine una aplicación de mapas que debe mostrarle al usuario información contextual de su ruta. Por un lado, una tarea consulta el estado del tráfico en tiempo real; por otro, una tarea distinta obtiene el clima actual de la zona. Como ambas operaciones son costosas e independientes entre sí, pueden ejecutarse de forma concurrente sin bloquear la interfaz de usuario, permitiendo que el proceso principal siga respondiendo a otras interacciones mientras espera los resultados.
...
El bloque `after` **evita que un proceso quede bloqueado indefinidamente** esperando un mensaje que quizá nunca llegue. Es el equivalente manual del tiempo de espera que `Task.await/2` ofrece de forma automática.
