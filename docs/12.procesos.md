...
En esta guía se explican los conceptos básicos sobre los procesos en Elixir y cómo manejarlos, con ejemplos prácticos de cada uno.
...
Para lograrlo, el sistema coordina y alterna la ejecución de las distintas tareas, de modo que todas puedan avanzar sin necesidad de que una termine completamente antes de que otra comience.
...
### Diagrama de Concurrencia vs Paralelismo
...
En otros lenguajes de programación, la concurrencia suele implementarse mediante **hilos tradicionales**, que comparten memoria y requieren mecanismos de sincronización complejos.
...
Un proceso de Elixir cumple entonces todo lo descrito anteriormente para un actor: estado privado, comunicación por mensajes a través de su *mailbox* y aislamiento frente a los fallos de los demás. Lo que aporta la BEAM es el **costo**:

* Son mucho más **ligeros** que los procesos del sistema operativo, ocupan apenas unos kilobytes en lugar de megabytes.
* Por eso se puede escalar hasta **miles o incluso millones** de procesos concurrentes en una sola máquina, muy por encima de lo que admiten los hilos tradicionales.

Es esa combinación (el modelo de actores a un costo casi nulo) la que hace a Elixir especialmente adecuado para construir **sistemas concurrentes y distribuidos**, como servidores web, aplicaciones de mensajería o sistemas que requieren alta disponibilidad.
...
Los procesos se crean con las funciones `spawn/1` y `spawn/3`.

**Ejemplo con `spawn/1`:**

Aquí hay un ejemplo simple que crea un proceso hijo que imprime un mensaje en la consola:

```elixir
IO.puts("Iniciando proceso...")

spawn(fn -> 
  IO.puts("Proceso hijo iniciado y finalizado!")
end)

IO.puts("Proceso principal finalizado.")
```

Aunque no es necesario definir un módulo para crear un proceso, es una buena práctica para organizar el código. El mismo ejemplo pero con un módulo definido:
...
defmodule MiModulo do
  def mi_funcion do
    ...
    IO.puts("Proceso hijo finalizado después de 2 segundos!")
  end
end
...
Es probable que el mensaje del proceso hijo no se imprima, porque el **proceso principal termina antes de que el hijo complete su tarea**. Lo que ocurre es esto:

![Diagrama de ciclo de vida de un proceso](res/ciclo_vida_proceso_principal_e_hijos.svg)

Para evitar esto, **se puede agregar una pausa en el proceso principal** para darle tiempo al proceso hijo de completarse. Modifique el código anterior agregando `:timer.sleep(3000)` justo antes de imprimir "Proceso principal finalizado." y vuelva a ejecutar el código para ver la diferencia.
...
Cada proceso tiene un **identificador único llamado PID** (*Process Identifier*), que se utiliza para referenciar y comunicarse con ese proceso. La función `spawn` devuelve el PID del proceso creado, así que se puede guardar en una variable para interactuar con él más adelante.
...
defmodule MiModulo do
  def mi_funcion(numero) do
    IO.inspect(self(), label: "Proceso hijo. Ejecutó la tarea #{numero}") # Muestra el PID del proceso actual
  end
end
...
En el diagrama del mailbox, varios procesos envían mensajes que se acumulan en orden y el actor receptor los consume uno a uno con pattern matching. Ahí se ve por qué no hay race conditions, ya que nunca hay dos mensajes procesándose al mismo tiempo dentro del mismo actor.
...
# Puede enviar más mensajes al mismo actor, pero solo el primero será procesado en este ejemplo
send(task1_pid, {:mensaje, "Segundo mensaje"})

# Mostrar mensaje de completado
IO.puts("Todas las tareas han terminado")
```

Ejecute este código y observe cómo el proceso principal (actor emisor) envía un mensaje al proceso `task1` (actor receptor), que lo recibe e imprime en la consola. El proceso hijo espera un solo mensaje y luego termina.
...
El módulo `Process` proporciona funciones para **inspeccionar, controlar y registrar procesos** dentro del entorno de ejecución de Elixir. Aunque normalmente trabajamos con procesos usando `spawn`, `send` y `receive`, este módulo ofrece un control más detallado.

Algunas funciones útiles del módulo `Process` incluyen:

- `Process.info/1` y `Process.info/2`: Obtienen información de un proceso. La versión `/1` devuelve todos los datos (estado, memoria, mensajes en la cola, etc.), mientras que la `/2` consulta un dato específico, como `Process.info(pid, :message_queue_len)`.
...
- `Process.whereis/1`: Devuelve el PID asociado a un nombre registrado (o `nil` si no existe).
...
defmodule DemoProcess do
  # Crea el actor y lo registra con el nombre :contador
  def iniciar do
    pid = spawn(fn -> loop() end)
    Process.register(pid, :contador)
    IO.puts("Proceso registrado como :contador con PID #{inspect(pid)}")
    pid
  end

  # Envía mensajes al contador
...
# Uso
pid = DemoProcess.iniciar()
DemoProcess.enviar_mensajes()
DemoProcess.inspeccionar(pid)
```

Para este ejemplo, se crea un proceso que actúa como un acumulador. Se registra con el nombre `:contador` para facilitar su referencia. Luego, se envían mensajes para sumar valores y mostrar el total acumulado. Finalmente, se inspecciona el estado del proceso utilizando funciones del módulo `Process`.

> ⚠️ **Ejecute este código varias veces y observe la salida.** El valor de `Mensajes pendientes` **cambia entre ejecuciones**: unas veces será `3` y otras `0`. No es un error, es concurrencia real: el proceso principal envía los tres mensajes y consulta la cola de inmediato, sin esperar. Según cómo el planificador de la BEAM reparta el tiempo de CPU, el actor alcanzará o no a procesarlos antes de la consulta.
>
> Esta es justamente la razón por la que **no se debe depender del orden de ejecución entre procesos**. Si un proceso necesita saber que otro ya terminó, debe enterarse **por un mensaje**, no por suposiciones de tiempo.
...
> ⚠️ **Importante: `Task.await/1` espera como máximo 5 segundos.** Si se omite el segundo argumento, el tiempo de espera por defecto es de **5000 milisegundos**. Si la tarea tarda más, el programa termina con el error:
>
> ```
> ** (exit) exited in: Task.await(..., 5000)
>     ** (EXIT) time out
> ```
>
> Cuando una tarea pueda tardar más que eso, indique explícitamente el tiempo de espera, por ejemplo `Task.await(task, 30_000)`, o `Task.await(task, :infinity)` para esperar sin límite.
...
Tanto `spawn` como el módulo `Process` y el módulo `Task` permiten crear y manejar procesos en Elixir, pero cada uno tiene sus propias características y casos de uso recomendados.
...
Los siguientes ejemplos usan procesos y concurrencia con `spawn` y con el módulo `Task`. Intente hacer su propia implementación de cada uno.

### Ejemplo 1

Imagine una aplicación de mapas que debe mostrarle al usuario información contextual de su ruta. Por un lado, una tarea consulta el estado del tráfico en tiempo real; por otro, una tarea distinta obtiene el clima actual de la zona. Como ambas operaciones son costosas e independientes entre sí, pueden ejecutarse de forma concurrente sin bloquear la interfaz de usuario, de modo que el proceso principal siga respondiendo a otras interacciones mientras espera los resultados.

Las funciones `tarea1` y `tarea2` se ejecutan de forma concurrente con `Task.async/1`, y cada una simula una operación costosa con `:timer.sleep/1`. El proceso principal sigue trabajando mientras tanto y solo se detiene al esperar los resultados.
...
Considere una aplicación que debe procesar varias operaciones costosas, como subir un conjunto de imágenes a una red social, donde cada imagen tarda un tiempo diferente según su tamaño. Si las subidas se ejecutan de forma secuencial, cada una debe esperar a que la anterior termine y el tiempo total se alarga. Si se ejecutan de forma concurrente, todas avanzan al mismo tiempo y el total se reduce al de la subida más lenta.

El ejemplo crea varias tareas y las ejecuta primero de forma secuencial y luego concurrente, para comparar los tiempos.
...
Se espera que la ejecución secuencial tome aproximadamente **15 segundos** (1+2+3+4+5), mientras que la concurrente debería tomar alrededor de **5 segundos**, el tiempo de la tarea más lenta, ya que todas se ejecutan al mismo tiempo.

> ⚠️ **Advertencia:** Si aumenta las duraciones, recuerde el límite de `Task.await/1`. **Si va a experimentar con tiempos mayores, use `Task.await(task, 30_000)`.**
...
Suponga un sistema con dos componentes que necesitan comunicarse: una tienda en línea que debe procesar un pago a través de un banco. El Proceso 1 representa el servicio del banco, encargado de validar y procesar la transacción. El Proceso 2 representa la tienda, que envía la solicitud al banco y espera la respuesta para confirmarle al cliente si el pago fue exitoso. De esta manera, la tienda puede lanzar la consulta al banco como una tarea asíncrona y esperar la respuesta sin bloquear otras operaciones de la aplicación.

En este ejemplo, en lugar de intercambiar mensajes manualmente con `send` y `receive`, el Proceso 2 (la tienda) **lanza al Proceso 1 (el banco) como una tarea con `Task.async`** y luego **espera su resultado con `Task.await`**. Es decir, `Task.async` crea un proceso que ejecuta una función y `Task.await` bloquea hasta recibir su valor de retorno, obteniéndose así un esquema de **solicitud–respuesta** sin gestionar el *mailbox* a mano. Aquí el "mensaje" que recibe el Proceso 1 es el argumento de la función, y la "respuesta" es el valor que esta retorna.

```elixir
defmodule EjemploComunicacionTask do
  @moduledoc """
  Ejemplo de un esquema solicitud–respuesta entre dos procesos en Elixir usando `Task`.
  El Proceso 2 lanza al Proceso 1 con `Task.async` y espera su valor de retorno con
  `Task.await`, sin usar `send` ni `receive` directamente.
  """
...
Asegúrese que su implementación cumpla con las características del Modelo de Actores:
...
### Ejercicio 2: Solicitud–respuesta con `after`

Escriba un script que cree dos procesos: uno que envíe un mensaje y otro que lo reciba, lo imprima y le devuelva una confirmación al emisor. Use `spawn`, `send`, `receive` y `after`. **No use `Task`**.

Investigue cómo funciona el bloque `after` dentro de `receive`, que permite definir qué hacer si **no llega ningún mensaje** en un tiempo determinado:

```elixir
receive do
  {:respuesta, texto} -> IO.puts("Llegó: #{texto}")
after
  2000 -> IO.puts("No hubo respuesta en 2 segundos")
end
```

El bloque `after` **evita que un proceso quede bloqueado indefinidamente** esperando un mensaje que quizá nunca llegue. Es el equivalente manual del tiempo de espera que `Task.await/2` ya trae incorporado.

Responda además:

1. ¿Qué ocurre si el proceso receptor nunca envía la confirmación y **no** se usa `after`?
2. ¿En qué casos conviene, en cambio, mantener un proceso esperando indefinidamente mediante un bucle recursivo, como en el ejemplo del módulo `Receiver`?
