...
Todo `GenServer` se organiza en dos mitades, como acaba de mostrar el ejemplo. Conviene separarlas en el archivo con un comentario, tal como se hizo arriba.
...
### 2. Callbacks del Servidor (invocados por OTP)

Estas funciones se declaran con `def`, pero **nunca se llaman directamente**. Las ejecuta el proceso del `GenServer` cuando le llega un mensaje.
...
Los cinco aparecieron trabajando juntos en el contador de la sección anterior.
