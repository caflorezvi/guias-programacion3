...
Ambos mantienen estado dentro de un proceso, así que la pregunta habitual es cuál de los dos usar. Estas son sus diferencias:
...
Como regla general, **`Agent` basta cuando lo único que se hace es guardar y recuperar un valor**. Conviene pasar a `GenServer` en cuanto aparece alguna de estas necesidades:
...
La tabla deja algo por fuera. **Los dos se integran igual de bien con supervisores**, porque un `Agent` es, por debajo, un `GenServer` con una interfaz reducida. Elegir uno u otro no compromete la tolerancia a fallos, que es el tema de la próxima guía.
