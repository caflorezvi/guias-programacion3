...
Tanto `Integer.parse/1` como `Float.parse/1` retornan un `tupla` con el número parseado y el resto de la cadena. Si no se puede parsear (convertir), retornan el átomo `:error`. Para manejar el resultado de manera efectiva, se utiliza un `case`.

Un `case` permite manejar diferentes patrones de manera concisa y legible. En este caso, se utiliza para evaluar el resultado de las funciones de parseo y proporcionar valores predeterminados en caso de errores. Este es un ejemplo de **pattern matching**.

Por último, se agrega la función `imprimir_error/1` para manejar los mensajes de error de manera centralizada. Observe que `IO.puts/2` tiene `:standard_error` que se utiliza para imprimir mensajes de error en la salida estándar de error (esto se hace por buenas prácticas).
