# Guías de apoyo para Programación III - Elixir

*Versión 1.0 - Junio 2026*

*Autor: Carlos Andrés Florez Villarraga*

[![Licencia: CC BY-SA 4.0](https://img.shields.io/badge/Licencia-CC%20BY--SA%204.0-green.svg)](https://creativecommons.org/licenses/by-sa/4.0/deed.es)
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.20754378.svg)](https://doi.org/10.5281/zenodo.20754378)

Este repositorio está dedicado a recopilar y organizar los recursos relacionados con el curso de **Programación III**, con un enfoque específico en la programación funcional utilizando **Elixir**. Aquí encontrarás documentos, guías de apoyo, ejemplos de código y otros materiales que te ayudarán a comprender mejor los conceptos y técnicas asociados con este lenguaje de programación.

Iniciando con una introducción a Elixir, exploraremos sus características principales, su sintaxis y cómo se diferencia de otros lenguajes de programación. A medida que avancemos, abordaremos temas como la concurrencia, el manejo de procesos, la programación funcional y el uso de herramientas específicas de Elixir.

## Contenido

| #  | Guía | #  | Guía |
|----|------|----|------|
| 01 | [Paradigmas de programación](docs/01.paradigmas-programacion.md) | 09 | [Streams](docs/09.streams.md) |
| 02 | [Introducción a Elixir](docs/02.introduccion-elixir.md) | 10 | [Archivos](docs/10.archivos.md) |
| 03 | [Programación funcional](docs/03.programacion-funcional.md) | 11 | [Procesos](docs/11.procesos.md) |
| 04 | [Conceptos básicos de Elixir](docs/04.conceptos-basicos-elixir.md) | 12 | [Nodos distribuidos (1)](docs/12.nodos-dist-1.md) |
| 05 | [Expresiones lógico-matemáticas](docs/05.expresiones-logico-matematicas.md) | 13 | [Nodos distribuidos (2)](docs/13.nodos-dist-2.md) |
| 06 | [Colecciones](docs/06.colecciones.md) | 14 | [OTP](docs/14.otp.md) |
| 07 | [Recursividad](docs/07.recursividad.md) | 15 | [Supervisores](docs/15.supervisores.md) |
| 08 | [Structs](docs/08.structs.md) | 16 | [Mix](docs/16.mix.md) |

📄 [Proyecto final](docs/0.proyecto-final.md)

## Ejemplos de código

En la carpeta [`ejemplos/`](ejemplos/) encontrarás código de apoyo organizado por temas:

- [Conceptos básicos](ejemplos/basicos)
- [Colecciones](ejemplos/colecciones)
- [Recursividad](ejemplos/recursividad)
- [Archivos](ejemplos/archivos)
- [Concurrencia](ejemplos/concurrencia)
- [Nodos distribuidos](ejemplos/nodos)
- [Talleres](ejemplos/talleres)

## Licencia

Esta obra está licenciada bajo una [Licencia Creative Commons Atribución-CompartirIgual 4.0 Internacional (CC BY-SA 4.0)](https://creativecommons.org/licenses/by-sa/4.0/deed.es).

---

Universidad del Quindío 💚
