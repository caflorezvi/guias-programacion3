...
El programa deberá ejecutarse y mostrar este mensaje en pantalla.

> ⚠️ Haga un análisis del problema y siga los pasos del pensamiento computacional para resolverlo. Recuerde que el pensamiento computacional implica hacer una abstracción del problema, descomponer el problema en partes más pequeñas y manejables, identificar patrones, y codificar la solución.
...
- Y en este caso, se debe invocar manualmente la función principal (main) del módulo.
...
El módulo `Biblioteca` contiene la función `main`, que es la que se invoca para mostrar el mensaje de bienvenida. Esto permite una mejor organización del código y facilita la reutilización de funciones en otros contextos. Es **necesario invocar** la función `main` para que el **programa se ejecute**, por lo que se debe incluir la llamada a `Biblioteca.main()` al final del archivo.

> ⚠️ En Elixir (y en la mayoría de lenguajes funcionales), no hay una palabra clave return como en Java o C.
El valor de la última expresión evaluada en la función es el valor que esta devuelve.
...
En este caso, la función `saludar` se define como privada (usando `defp`), lo que significa que solo puede ser llamada desde dentro del módulo `Biblioteca`. Esto es útil para **encapsular la lógica que no necesita ser expuesta** públicamente. La función `main` actúa como el punto de entrada del módulo y se encarga de llamar a la función privada `saludar`. Esto permite una mejor organización del código y facilita la reutilización de funciones en otros contextos.
...
defmodule Biblioteca do
  ...
  defp saludar(nombre) do
    IO.puts("Hola #{nombre}, Bienvenido al sistema de la Biblioteca Central")
  end
end
...
En este caso, la función `saludar` ahora acepta un parámetro `nombre`, que se utiliza para personalizar el mensaje de bienvenida. Esto permite que el programa sea más flexible y pueda adaptarse a diferentes usuarios. En este caso, el nombre del usuario se pasa como argumento al llamar a la función `saludar` desde `main` y se **interpola** en el mensaje de bienvenida.
...
En este caso, la función `IO.gets/1` se utiliza para **mostrar un mensaje al usuario y esperar su entrada**. La entrada se almacena en la variable `nombre`, que luego se pasa a la función `saludar`. 
...
La función `leer_nombre` ahora está usando el operador pipe `|>` para **encadenar** la llamada a `String.trim/1`, básicamente la salida de `IO.gets/1` se pasa directamente a `String.trim/1` y este último se encarga de eliminar cualquier espacio en blanco o salto de línea al principio y al final de la cadena. Esto garantiza que el nombre ingresado por el usuario se procese correctamente antes de ser utilizado en el saludo.

Incluso, podemos usar el operador pipe `|>` en la función `main` así:
...
- Revisar la [documentación](https://elixirschool.com/es/lessons/basics/documentation) oficial de Elixir.
- Revisar el [código fuente de Kernel](https://github.com/elixir-lang/elixir/blob/main/lib/elixir/lib/kernel.ex) para que observe cómo se documentan las funciones y módulos.
